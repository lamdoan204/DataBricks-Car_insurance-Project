...
The repository is organized into several directories, with each directory
serving a specific role in the end-to-end Data Engineering pipeline.

```text
DataBricks-Car_insurance-Project/
│
├── code_scripts_on_Databricks/
│   ├── 1_ingestions/  -- code ingesttion from multiple sources
│   │   └── Data ingestion scripts and notebooks
│   │
│   └── 2_transformations/ -- code transformation
│       └── Data transformation and processing scripts
│
├── images/ -- images for proof and architecture image
│   └── Project documentation, architecture diagrams, and screenshots
│
├── object_data/ 
│   └── Object and image data used by the project
│
├── postgres/ 
│   ├── data/ 
│   │   └── PostgreSQL source data
│   │
│   └── sql_scripts/ -- code sql create, insert data and grant permission for user
│       └── SQL scripts for database creation and data preparation
│
├── producer/
│   ├── data/
│   │   └── Data used by the Kafka producer
│   │
│   └── main.py -- code emulation producer send messages to kafka
│       └── Kafka producer implementation
│
├── .gitattributes
├── .gitignore
├── Note - Data Engineer Project.docx
├── README.MD
├── build.sh -- bash build.sh init create kafka brokers, topic and starting send message to kafka 
└── docker-compose.yml 
```
## 🏗️ Architecture
...
---
### 6. Data Quality During Transformation

Data quality checks are applied directly during the data transformation
process to ensure that the data meets the expected business and technical
requirements before being passed to downstream layers.

The project uses **Databricks Delta Live Tables (DLT) expectations** to
define validation rules for transformed data.

The quality constraints cover several aspects of the data, including:

- Required fields must not contain `NULL` values
- Data types must match the expected schema
- Customer IDs must be unique
- Names are validated and normalized
- Date fields must contain valid date values
- Location fields are validated and cleaned
- ZIP codes must contain valid integer values

For example, the following DLT expectations are applied during the
transformation process:
![data quality](./images/proof/quality_contrains_data_with_dlt.png)

### 7. Medallion Architecture

The data platform follows the **Medallion Architecture**, which separates
data processing into three logical layers manage by unity catalog:
![unity catalog](./images/proof/Medallion_Architecture.png)
### 8. Analytics Dashboard

After completing the data ingestion, transformation, and data quality
processes, the processed data from the **Gold layer** is used to build
an analytics dashboard in Databricks.

The dashboard provides a high-level overview of the processed insurance
data and demonstrates how the data platform can support downstream
analytics and business insights.

The dashboard is built on top of the curated datasets produced by the
data pipeline, allowing the analytics layer to consume data that has
already gone through the Bronze → Silver → Gold transformation process.

![Analytics Dashboard](./images/proof/Dashboard.png)
